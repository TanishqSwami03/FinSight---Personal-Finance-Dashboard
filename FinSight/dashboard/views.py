...
def buy_stock(request):
    if request.method == 'POST':
        ...
        # Get the user's wallet balance
        user = request.user
        wallet_balance = user.wallet_balance  # Assuming you have a wallet model related to the user
        ...
        portfolio, created = Portfolio.objects.get_or_create(
            ...
            defaults={'quantity': 0, 'purchase_price': price_per_share, 'current_price': price_per_share}
        )
        ...
        if not created:
            ...
            # If it's a new stock, set the purchase price and quantity
            portfolio.quantity = quantity
            portfolio.purchase_price = price_per_share

        portfolio.current_price = price_per_share  # Update current price
        portfolio.save()
        ...
        Transaction.objects.create(
            user=user,
            transaction_type='buy_stock',  # You can name this anything meaningful like 'buy_stock'
            amount=total_amount
        )
